...
class SaverMixin():
    ...
    DEFAULT_GRAYSCALE_KWARGS = {'data_range': None, 'cmap': 'jet'} # 'jet' None
    ...
    def get_image_grid_(self, imgs,rows=2):
        ...
        if rows>1: # multiple rows visualization
            n_col = math.ceil(len(cols)/rows) 
            append_n_imgs= n_col*rows - len(cols)
            for idx in range(append_n_imgs):
                cols.append(np.zeros_like(cols[0]))
            row_cols=[]
            for idx in range(rows):
                row_cols.append(np.concatenate(cols[idx*n_col:(idx+1)*n_col], axis=1))
            return np.concatenate(row_cols, axis=0)
        return np.concatenate(cols, axis=1)
    
    def save_image_grid(self, filename, imgs, rows=2):
        img = self.get_image_grid_(imgs, rows)
        cv2.imwrite(self.get_save_path(filename), img)
        img_infos = []
        for idx, img_info in enumerate(imgs):
            if "name" in img_info.keys():
                img_infos.append({"name":img_info['name'],"idx":idx})
        self.save_json(filename.replace(".png",".json"),img_infos)
