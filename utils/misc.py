...
def load_config(*yaml_files, cli_args=[]):
    ...
    if 'dataset' in cli_conf.keys():
        if 'scene' in cli_conf['dataset'].keys() and cli_conf['dataset']['scene'] is not None:
            if cli_conf['dataset']['scene'][:4]=='co3d':
                cli_conf['dataset']['scene']=cli_conf['dataset']['scene'][4:]
