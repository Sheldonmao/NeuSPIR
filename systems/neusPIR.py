...
@systems.register('neusPIR-system')
class NeuSPIRSystem(BaseSystem):
    """ NeuS-PIR system
    ...
    def preprocess_data(self, batch, stage):
        ...
        if stage in ['train']:
            if self.config.model.background == 'white':
                self.model.background_color = torch.ones((12,), dtype=torch.float32, device=self.rank)
            elif self.config.model.background == 'random':
                self.model.background_color = torch.rand((12,), dtype=torch.float32, device=self.rank)
            elif self.config.model.background == 'black':
                self.model.background_color = torch.zeros((12,), dtype=torch.float32, device=self.rank)
