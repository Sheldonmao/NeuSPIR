...
def get_bbox_intersection(
    cam_loc: Float[Tensor,"n_pts 3"],       # camera origin location, rays_o
    ray_directions: Float[Tensor,"n_pts n_dirs 3"],     # rays directions, rayd_d
    scene_aabb:Float[Tensor, "6"] = torch.tensor([-1.,-1.,-1.,1.,1.,1.]) # scene bounding box for -x,-y,-z,x,y,z
) -> (
     Float[Tensor,"n_pts n_dirs 2"], # near far dist along direction
     Bool[Tensor,"n_pts n_dirs"] # intersection mask
):  
    scene_aabb = scene_aabb.to(cam_loc.device)
    min_x = (scene_aabb[0] - cam_loc[:,0]).unsqueeze(1)/ray_directions[:,:,0]
    min_y = (scene_aabb[1] - cam_loc[:,1]).unsqueeze(1)/ray_directions[:,:,1]
    min_z = (scene_aabb[2] - cam_loc[:,2]).unsqueeze(1)/ray_directions[:,:,2]
    max_x = (scene_aabb[3] - cam_loc[:,0]).unsqueeze(1)/ray_directions[:,:,0]
    max_y = (scene_aabb[4] - cam_loc[:,1]).unsqueeze(1)/ray_directions[:,:,1]
    max_z = (scene_aabb[5] - cam_loc[:,2]).unsqueeze(1)/ray_directions[:,:,2]

    dist_values = torch.stack([min_x,min_y,min_z,max_x,max_y,max_z],dim=-1)
    intersection_mask = (min_x*max_x<0) & (min_y*max_y<0) & (min_z*max_z<0)
    
    temp_dist = dist_values.clone()
    temp_dist[temp_dist<0] = torch.inf
    far,_ = torch.min(temp_dist,dim=-1)

    temp_dist = dist_values.clone()
    temp_dist[temp_dist>0] = -torch.inf
    near,_ = torch.max(temp_dist,dim=-1)

    intersection = torch.stack([near,far],dim=-1)
    intersection = intersection.clamp_min(0.01)
    return intersection,intersection_mask

class RayTracing(nn.Module):
    def __init__(
            self,
            scene_aabb:Float[Tensor, "6"] = torch.tensor([-1.,-1.,-1.,1.,1.,1.]),
    ...
    def forward(self,
                sdf,                                    # function to query sdf value
                cam_loc: Float[Tensor,"n_pts 3"],       # camera origin location, rays_o
                object_mask: Bool[Tensor,"n_pts*n_dirs"],   # valid hit mask, initialized as ones
                ray_directions: Float[Tensor,"n_pts 3"]     # rays directions, rayd_d
                ):

        batch_size, num_pixels, _ = ray_directions.shape
        # TODO: get bbox intersection implimentation
        bbox_intersections, mask_intersect = get_bbox_intersection(cam_loc, ray_directions, scene_aabb=self.scene_aabb)
        curr_start_points, unfinished_mask_start, acc_start_dis, acc_end_dis, min_dis, max_dis = \
            self.sphere_tracing(batch_size, num_pixels, sdf, cam_loc, ray_directions, mask_intersect, bbox_intersections)
        ...
        if mask_left_out.sum() > 0:  # project the origin to the not intersect points on the sphere
            ...
            acc_start_dis[mask_left_out] = -torch.bmm(rays_left_out.view(-1, 1, 3), cam_left_out.view(-1, 3, 1)).squeeze().float()
    ...
    def sphere_tracing(self, batch_size, num_pixels, sdf, cam_loc, ray_directions, mask_intersect, bbox_intersections):
        ''' Run sphere tracing algorithm for max iterations from both sides of unit sphere intersection '''
        bbox_intersections_points = cam_loc.reshape(batch_size, 1, 1, 3) + bbox_intersections.unsqueeze(-1) * ray_directions.unsqueeze(2) # (n_pts, n_dirs, 2, 3)
        ...
        curr_start_points[unfinished_mask_start] = bbox_intersections_points[:,:,0,:].reshape(-1,3)[unfinished_mask_start]
        acc_start_dis = torch.zeros(batch_size * num_pixels).cuda().float()
        acc_start_dis[unfinished_mask_start] = bbox_intersections.reshape(-1,2)[unfinished_mask_start,0]

        # Initialize end current points
        curr_end_points = torch.zeros(batch_size * num_pixels, 3).cuda().float()
        curr_end_points[unfinished_mask_end] = bbox_intersections_points[:,:,1,:].reshape(-1,3)[unfinished_mask_end]
        acc_end_dis = torch.zeros(batch_size * num_pixels).cuda().float()
        acc_end_dis[unfinished_mask_end] = bbox_intersections.reshape(-1,2)[unfinished_mask_end,1]
